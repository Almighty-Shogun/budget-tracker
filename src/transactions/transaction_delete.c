...
enum menu_option {
    ...
    DELETE_OLDER_TRANSACTION = 2,
    BACK_TO_MAIN_MENU = 9
};

/**
 * @brief Displays the "Delete transaction" menu with options for user actions.
 *
 * @author Almighty-Shogun
 */
static void show_menu() {
    menu_show_header("Delete a transaction");
    ...
    menu_show_option(DELETE_OLDER_TRANSACTION, "Delete an older transaction");
    menu_show_option(BACK_TO_MAIN_MENU, "Go back to the main menu");
    console_write_line(nullptr);
}

/**
 * @brief Deletes a transaction by ID for a given year and month.
 *
 ...
 * @author Almighty-Shogun
 */
static void delete_transaction(int year, const char* month) {
    if (year <= 0) {
    ...
    console_write_line("Deleting a transaction of " CONSOLE_COLOR_CYAN "%s %d", month, year);
    console_write_line(nullptr);
    ...
    if (!transaction_json) {
        console_write_line(CONSOLE_COLOR_RED "Failed to get the transaction.");
        return;
    }
    ...
    if (!input_ask_yes_or_no("Are you sure you want to delete this transaction?", true)) {
        console_write_line(CONSOLE_COLOR_GREEN "Transaction deletion cancelled.");
        return;
    }

    cJSON_DeleteItemFromArray(json, transaction_index);
    ...
    console_write_line(nullptr);
    console_write_line(CONSOLE_COLOR_GREEN "Transaction deleted successfully.");
}

/**
 * @brief Displays the help menu for the "Delete a transaction" section.
 *
...
static void handle_help_option(const int year, const char* month) {
    menu_show_header("Delete a transaction - Help menu");
    console_write_line(nullptr);

    menu_show_option(DELETE_TRANSACTION, "Delete a transaction");
    console_write_line("Deletes a transaction of " CONSOLE_COLOR_CYAN "%s %d", month, year);
    console_write_line(nullptr);

    menu_show_option(DELETE_OLDER_TRANSACTION, "Delete an older transaction");
    console_write_line("Deletes an older transaction.");
...
/**
 * @brief Handles the "Delete a transaction" option.
 *
 ...
 * @author Almighty-Shogun
 */
static void handle_delete_transaction_option(const int year, const char* month) {
    delete_transaction(year, month);
}

/**
 * @brief Handles the "Delete an older transaction" option.
 *
 * @author Almighty-Shogun
 */
static void handle_delete_older_transaction_option() {
    ...
    do {
        ...
        switch (choice) {
            ...
            case DELETE_TRANSACTION: {
                handle_delete_transaction_option(current_year, current_month);
                break;
            }
            case DELETE_OLDER_TRANSACTION: {
                handle_delete_older_transaction_option();
                break;
            }
